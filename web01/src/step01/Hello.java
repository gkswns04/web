...
- 예) 인사관리시스템, 물류관리시스템, 고객관계관리시스템 등 관리시스템 류.
...
- 메서드
  init(): 
      서블릿 컨테이너가 서블릿을 생성을 생성한 후 호출한다. 
      서블릿을 실행하는데 필요한 자원을 준비시키는 코드를 넣는다.
  service(): 
      클라이언트가 요청할 때 마다 호출한다.
      클라이언트 요청을 처리하는 코드를 둔다.
  destroy():
      웹 애플리케이션이 멈출 때 호출한다.
      서블릿이 사용한 자원을 해제시키는 코드를 둔다.
  getServletInfo():
      서블릿에 대한 간단한 정보를 문자열로 리턴한다.
      서블릿 컨테이너에서 서블릿 관리할 때 서블릿의 정보를 출력하기 위해 호출한다.
      일반적으로 요청을 처리하는 작업과는 무관한다.
  getServletConfig():
      서블릿의 설정 정보를 다루는 객체(ServletConfig)를 리턴한다.
      ServletConfig 객체는 서블릿 컨테이너가 init()를 호출할 때 파라미터로 넘겨준다.
      이 메서드도 요청을 처리하는 작업과는 무관한다.
...
- conf: 톰캣 서버의 설정 파일을 모아 둔 디렉토리
- lib: 톰캣 서버 및 톰캣 서버를 실행하는데 필요한 자바 라이브러리를 모아둔 디렉토리 
- logs: 톰캣 서버의 실행 상황을 기록한 로그 파일을 모아둔 디렉토리
        톰캣 서버에 문제가 발생했을 때 이 로그 파일을 분석하여 문제를 찾는다.
- temp: 임시 작업 결과물을 두는 디렉토리
- webapps: 웹 애플리케이션을 모아 둔 디렉토리.
           웹 애플리케이션을 배포할 때 기본으로 이 디렉토리에 두어야 한다.
           물론 다른 디렉토리에 웹 애플리케이션을 두고,
           톰캣 설정 파일에서 그 디렉토리를 지정할 수 있다.
...
- 웹 애플리케이션 디렉토리
  $웹애플리케이션/WEB-INF/web.xml    : 웹 애플리케이션 설정 파일이다.
                         /classes    : 자바 바이트코드(.class) 및 실행할 때 사용 할
                                       설정 파일(.xml, .properties 등)을 둔다.
                         /lib        : 자바 라이브러리 파일(.jar)을 둔다.
                 /...                : 디렉토리, HTML, CSS, JavaScript, JSP, 
                                       이미지 파일 등 웹 자원을 둔다.
  주의!
  => 절대 WEB-INF에 HTML, CSS 등 웹 자원을 두지 말라!
  => 클라이언트에서 사용할 수 없다.
...
- 이 파일을 "배치기술서(Deployment Descriptor)"라 부른다.
