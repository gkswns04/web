...
2) POST 요청
  => 서블릿 컨테이너의 종류에 상관없이, 요청 데이터를 꺼내기 전에 
     파라미터 값이 어떤 문자집합으로 인코딩 되었는지 다음과 같이 지정하면 된다.
...
=> 이유?
   초창기 네트워크 장비가 7비트를 기준으로 만들어져있다.
=> 한글은 8비트를 두 개 또는 세 개 사용하여 표현한다.
...
=> 해결책?
   ...
   ASCII 7비트: "%EA%B0%80" (25 45 41 25 42 30 25 38 30) 
               --> 00100101 01000101 01000001 00100101 ...
=> ASCII 코드는 무조건 7비트이기 때문에 안전하게 다른 장비로 데이터를
   보낼 수 있다.
=> URL 인코딩은 7비트 장비에 들어갔을 때 데이터가 짤릴 수 있는 경우에 사용한다.
=> 웹 브라우저는 웹 서버에 데이터를 보낼 때, 
   한글과 같이 8비트 모두를 사용하는 데이터에 대해서는 URL 인코딩 하여 보낸다.
=> 웹 서버는 URL 인코딩 데이터를 원래의 데이터로 자동 변환한다.
=> 개발자가 URL 인코딩과 URL 디코딩을 할 필요는 없다.
=> 그러나 URL 인코딩이 뭔지는 이해해야 한다.
   
# 요청 프로토콜 예)   
POST http://localhost:8080/web02/step04/servlet17 HTTP/1.1
Host: localhost:8080
...
name=%ED%95%9C%EA%B8%80&email=han%40test.com&tel=111-1111      
